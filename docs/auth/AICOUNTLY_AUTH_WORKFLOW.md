# AICOUNTLY auth workflow (Capital)

How Capital signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `capital.aicountly.com` (or `capital.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/capital?returnUrl={origin}/auth/callback`.
...
| Capital host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `capital.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `capital.aicountly.com/api` |
| `capital.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `capital.gh.aicountly.com/api` |
...
`capital` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://capital.gh.aicountly.com/api/health
...
curl -i -X POST https://capital.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://capital.gh.aicountly.com/api/global/login   # expect 404
```
